...
int main(int argc, char *argv[]) {
  ...
  printf("Initial state: n: %d, num_procs: %d, my_work: %d, elms_to_comm: %d\n",n, num_procs, my_work, elms_to_comm );
  ...
  if (my_rank == ROOT) {
    ...
    /* Use MPI_Send */
	  for (int i = 0; i < num_procs; i++) {
		  ...
		  for (int j = 0 ; j < n;j++) {
		  	printf("%d", my_work);
	  		MPI_Send(&A[i*my_work+j], elms_to_comm, MPI_DOUBLE,i,my_rank, MPI_COMM_WORLD);
		  }
    ...
    /* Use MPI_Recv */
	  for (int i = 0 ; i < num_procs; i++) {
	  	...
	  	MPI_Recv(&A[j], elms_to_comm, MPI_DOUBLE,i, my_rank, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
		  }
...
// COllective
  ...
  for (int i = 0; i < num_procs; i++) {
  	...
  	MPI_Bcast(&B[j], my_work, MPI_DOUBLE, my_rank, MPI_COMM_WORLD);
	  }
  }
  ...
  if (my_rank == ROOT) {
    ...
    /* Use MPI_Recv */
	  for (int i = 0; i < num_procs; i++) {
	  	...
	  	MPI_Recv(&C[j], my_work,MPI_DOUBLE,i, ROOT,MPI_COMM_WORLD, MPI_STATUS_IGNORE );
		  }
    ...
    /* Use MPI_Send */
	  ...
	  for (int i = 0; i<num_procs; i++) {
	  	...
	  	MPI_Send(&C[j],my_work, MPI_DOUBLE, ROOT,flag, MPI_COMM_WORLD);
		  }
