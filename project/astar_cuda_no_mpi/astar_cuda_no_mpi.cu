...
void output_tail(int* graph, int graph_size, int tail_count) {
     ...
     for (int i = 5 ; i >= 1; i--) {
     	 printf("%*d", 3, graph[no_of_nodes-i]);
     }
     printf("\n");
}
...
__global__ void cal_heuristic(int * d_hList, int grid_size, uint64_t end, int heuristic_type) {
	   uint64_t i = blockDim.x* blockIdx.x + threadIdx.x;
	   int row_diff, col_diff;
	   if (i < grid_size*grid_size) {
	      row_diff = end/grid_size-i/grid_size;
	      if (row_diff < 0)
	      	 row_diff = -1*row_diff;
	      col_diff = end%grid_size -i%grid_size;
	      if (col_diff < 0)
	      	 col_diff = -1*col_diff;
	      if (heuristic_type == 0) {
...
__global__ void astar_kernel(int * d_openList, int * d_closedList, int * d_o_array, int * d_gList, int * d_hList,int * d_neighbor, int * nVV,int grid_size) {
	       int i = blockDim.x * blockIdx.x + threadIdx.x;
	       int g_score, nb;
	       if (i< grid_size*grid_size && d_openList[i] == 1) {
	       	  *nVV =1;
		  d_openList[i] = 0;
		  d_closedList[i] = 1;
		  //printf("graph_index: %d \n", i);
		  for (int j = 0; j < 4; j++) {
		      nb = i + d_neighbor[j];
