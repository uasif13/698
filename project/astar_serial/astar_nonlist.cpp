...
int astar(Graph * graph, string heuristic) {
  ...
  int * open = new int[grid_size*grid_size];
  ...
  while (open_size != 0) {
    // lowest f
    int index_f = open.top();
    int index_f_score = gList[index_f] + hList[index_f];
	...
	continue;
      ...
      if (auto iter = closed.find(nb); iter != closed.end()) {
	continue;
      }
      if (o_array[nb] == 1) {
	continue;
